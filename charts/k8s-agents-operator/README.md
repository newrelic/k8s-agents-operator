...
Please set `admissionWebhooks.autoGenerateCert.enabled` to `false` and `admissionWebhooks.autoGenerateCert.recreate` to `false `in your `values.yaml` file and follow the steps in the `cert-manager` section or provide your own certificate and key files.
