...
function create_cluster() {
    ...
    for label in $(find ${SCRIPT_PATH}/apps -type f -name '*.yaml' -exec yq '. | select(.kind == "Deployment") | .metadata.name' {} \;); do
      kubectl wait --timeout=120s --for=jsonpath='{.status.phase}'=Running --namespace $app_ns -l="app=$label" pod
    done
}
...
function test_dotnet() {
  local dotnet_resources_mem=$(kubectl get pods -l app=dotnetapp -n $app_ns -o yaml | yq '.items[] | .spec.initContainers[].resources.requests.memory')
