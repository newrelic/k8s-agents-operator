...
var (
	_                         webhookhandler.PodMutator = (*InstrumentationPodMutator)(nil)
	...
	instrumentationMutatorLog                           = ctrl.Log.WithName("instrumentation-mutator")
)
...
func (pm *InstrumentationPodMutator) Mutate(ctx context.Context, ns corev1.Namespace, pod corev1.Pod) (corev1.Pod, error) {
	logger := instrumentationMutatorLog.WithValues("namespace", pod.Namespace, "name", pod.Name, "generate_name", pod.GenerateName)
	...
	if err != nil {
		if errors.Is(err, errMultipleInstancesPossible) {
			instrumentationMutatorLog.Info("too many New Relic Instrumentation instances for this Pod.  only 1 allowed")
		} else {
			logger.Error(err, "failed to select a New Relic Instrumentation instance for this Pod")
		}
		return pod, err
