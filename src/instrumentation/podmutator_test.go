...
func TestMutatePod(t *testing.T) {
	...
	tests := []struct {
					...
					InitContainers: []corev1.Container{
						{
							...
							Command: []string{"cp", "/javaagent.jar", "/newrelic-instrumentation/javaagent.jar"},
							VolumeMounts: []corev1.VolumeMount{{
								Name:      volumeName,
								MountPath: "/newrelicinstrumentation",
							}},
						},
					},
					Containers: []corev1.Container{
		...
		},
		{
			...
			err: `instrumentations.newrelic.com "doesnotexists" not found`,
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err = mutator.Mutate(context.Background(), test.ns, test.pod)
			if test.err == "" {
				require.NoError(t, err)
